...
class GANManager:
    ...
    def generate_image(self, step_speed=0.05, mlp_latent_vector=None) -> np.uint8:
        ...
        1. Random Walk (movimento base fluido)
        2. MLP Mood (direzione semantica basata sull'audio)
        3. Audio Reactivity (volume/kick per impulsi ritmici)
        """

        with torch.no_grad():
            
            # 2. LOGICA DI NAVIGAZIONE BASE (Random Walk)
            # Aggiorniamo self.current_z per garantire un movimento continuo di sottofondo
            base_speed = 0.005 
            dynamic_speed = min(step_speed * 0.1, 0.5) 
            step = base_speed + dynamic_speed

            # Interpolazione lineare verso il target casuale (il "vagare")
            self.current_z = (1 - step) * self.current_z + step * self.target_z

            # Gestione cambio target random
            distance_to_target = torch.norm(self.target_z - self.current_z)
            if distance_to_target < 0.2:
                self.target_z = torch.randn(1, self.latent_dim).to(self.device)

            # 3. INTEGRAZIONE MLP (Il Mood della canzone)
            # Creiamo un vettore temporaneo per questo frame, partendo dalla posizione base
            final_z = self.current_z.clone()

            if mlp_latent_vector is not None:
                # Convertiamo l'array numpy dell'MLP in Tensore PyTorch
                # mlp_latent_vector shape attesa: (512,) -> Diventa (1, 512)
                mlp_tensor = torch.tensor(mlp_latent_vector, dtype=torch.float32, device=self.device).unsqueeze(0)
                
                # Sommiamo il mood alla posizione corrente.
                # L'MLP ci dice "dove spostarci" rispetto alla base per visualizzare quel mood.
                final_z = final_z + mlp_tensor

            # 4. GESTIONE IMPULSI ("Kick")
            # Aggiunge reattività immediata ai colpi di batteria forti
            if step_speed > 0.5:
                kick_impact = torch.randn(1, self.latent_dim).to(self.device) * step_speed * 0.2
                final_z += kick_impact

            # Normalizzazione sferica (Cruciale per StyleGAN)
            # Riporta il vettore sulla superficie dell'ipersfera per evitare artefatti
            final_z = final_z / final_z.norm() * np.sqrt(self.latent_dim)

            # 5. GENERAZIONE DELL'IMMAGINE
            # Usiamo final_z invece di self.current_z
